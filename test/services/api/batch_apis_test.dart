...
void main() {
  group('SoundCloudEmbedApi', () {
    test('constructUrl with params', () {
      const api = SoundCloudEmbedApi(640, soundCloudParams: SoundCloudEmbedParams(autoPlay: true));
  ...
  group('SpotifyEmbedApi', () {
    test('constructUrl', () {
      ...
      final uri = api.constructUrl('https://spotify.com/track/1', queryParameters: {'custom': '1'});
  ...
  group('TikTokEmbedApi', () {
    ...
    test('handleErrorResponse always returns EmbedApisException', () {
      const api = TikTokEmbedApi();
      expect(api.handleErrorResponse(http.Response('', 404)), isA<EmbedApisException>());
    });
  });
  ...
  group('VimeoEmbedApi', () {
    test('constructUrl and modifier', () {
      ...
      final uri = api.constructUrl('https://vimeo.com/1', queryParameters: {'a': 'b'});
      expect(uri.queryParameters['loop'], equals('true'));
      expect(uri.queryParameters['a'], equals('b'));
      
      final data = EmbedData(html: '<iframe src="//player.vimeo.com"></iframe>');
  ...
  group('XEmbedApi', () {
    test('constructUrl with locale and brightness', () {
      ...
      final uri = api.constructUrl('https://x.com/post/1', locale: 'ms', brightness: Brightness.dark);
    ...
    test('handleErrorResponse handles 403', () {
      const api = XEmbedApi();
      expect(api.handleErrorResponse(http.Response('', 403)), isA<EmbedDataRestrictedAccessException>());
    });
  });
}
